#' Extracting event data from the UTD real-time Event database with reference classes
...
#' @export
#' @examples Table$pullData(api_key=" ", table_name="Phoenix", country=list("USA","MEX","SYR","CHN"),
  ...
  methods = list(
    pullData = function(table_name, country, start, end) {
      if (table_name=="Phoenix") {
        ...
        date_constraint = list('date8'=list('$gte'=start,'$lte'=end))
