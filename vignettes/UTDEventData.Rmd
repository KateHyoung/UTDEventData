...
The R library, `UTDEventData`, provides the direct access to the political event database at the University of Texas at Dallas (UTD). Using this library, researchers can query the event data stored at the UTD's API server and load them into R for subsequent analysis. Several functions are prepared in the library to extract and to analize the data according to users' preference. This library also provides the citation functions ont only for data tables but also the library itself. 
...
To cite the package, an excution function `citation()` can be used, and users can find the citatation texts when using the data obtaining functions, `pullData()` and `sendQuery()` with a defualt option of `citation = TRUE`. Moreover, the function of `citeData()` in the package will print data ciatation texts in a R console. 
```{r, eval = FALSE}
# to cite the package 
...
`previewData()` shows 100 observations of a data table as an example. Occasionally, users need to know a pattern of variables or a data structure to analyze the data in R dataframes. This function can be useful when users want to see some pattenrs of an interesting variable to build a query block with a regular expression (`returnRegExp()`) function. 
...
The function returns citation texts \& BibTeX for publication at the end of data retrieval. To avoid printing them in your R-console, please turn off the `citation` option by logiscally choosing "FALSE" in the function. 
...
Throwing an API key into functions evvery time could be annoying tasks for users. To avoid troublesome work, we suggest some convenient approaches. 
...
Theird option is using a reference class, named `Table()` , prepared in this library. One downsiade of this is that the function works only with `DataTables()`, `tableVar()`, and `pullData()`. 
```{r, eval=FALSE}
# creating an object
obj <- Table$new()
...
Someone may want to download the entire data of a certain data table. `entireData()` helps users to do this as specifying a data table into the function. This function returns a dataframe of the requested data with the package citation. The data are directly stored to your disk, so make sure that your device has enough space to store them. The size of data can be estimated by `getQuerySize( )` with the  `entire` options. More detials are illustrated in a hlep page of `getQuerySize()`. 

Some data tables are greater than 10GB, so it may occur R errors if a local devise does not have enoguth space. If it is the case, please increase your memory allocated to your R. The large data usually takes a longer time to request and download them, so please check your memory and disk limits before obtaining the entire data.  
...
+ Extracting data with contries and time can be acheived with two ways in this package: 1) `pullData()` and 2) `sendQuery()`. This exmaple shows how to complete the following task with the two methods. 
