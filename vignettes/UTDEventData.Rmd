...
The aggregation functions in the package help users to extract and aggreated data with extendable queries as users want. While `pullData()` function provides limited query options such as country names and time ranges of a data set, the aggregation functions provide users more destcritions of query blocks such as contry names, time ranges, dyads of actors, latitute and longitude of event locations and source and targets indications of certain events. By using saved queries blocks with specific functions, user can retrieve data with constructed aggreation queries as combining aggregation functions in this package. To obtain aggregated data, user must acquaint each function's usage illustrated in the help pages. The details how individual function works together with the other functions will be illustrated in this section.  

The aggregation functions in the package consist of three groups. The first group of aggregation functions is creating qeury blocks of user's interests. The aggregating query blocks are the basis of aggregations information such as country names, locations (latitude and longitude), time ranges, and so on. Moreover, a user can use any vriable in an API table with a regural expression function. The second group of aggregtaion functions is connective functions of query blocks such as `orList()` and `andList()`. The third part of aggregation functions is requesting data based on the query a user creates with the combination of query blocks and linking functions.   
...
The data are retreived with the combinations generated with aforementioned functions. The `sendQuery()` function requests the API server to obtain the data in the structured format in R. A user should specified an API key, a data table, and a lists of queries in the `list()` format. 
